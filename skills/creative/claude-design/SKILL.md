...
- news flyers / article cards / social-post style posters built from real article content and imagery
...
### News flyer / article-card subtype

When the user wants a flyer or poster based on news articles:

- Use a real news image from the article page when available (`og:image` or the article's primary image).
- Keep the composition to one article per PNG unless the user explicitly asks for a carousel or multipage asset.
- Make the headline short and punchy; compress the story into a Gen-Z style summary when requested.
- Omit source text inside the flyer if the user says no source.
- Verify the render visually before sending, and reject typography-only outputs if the task requires an image-backed flyer.
- If Telegram delivery is requested, send each PNG as a separate file/photo rather than bundling multiple flyers into one asset.
- If Seedance is used for the visual layer, treat the HTML as a fixed template and ask Seedance to fill only the transparent/image slot; keep lower text panels and typography outside the model output.
- For promo/ad flyers, prefer a small A/B-test-ready package: multiple headline variants first, then optional subheadline and CTA variants. Keep the main offer, price, and CTA stable across headline tests so performance differences are attributable to the hook.
- If the user asks to make the CTA on a flyer clickable, do not imply the PNG itself can do that. Deliver the normal PNG plus a clickable HTML wrapper and/or linked SVG, and state the limitation plainly.

See `references/news-flyer-workflow.md` for the reusable workflow.
See `references/clickable-promo-flyers.md` for the PNG-vs-clickable-wrapper rule, local SVG→PNG export fallback, common ad sizes (`1080x1350`, `1080x1080`, `1920x1080`), and hard-selling promo-banner pattern.
See `references/seedance-template-guidance.md` for the Seedance prompt pattern.
Use `templates/seedance-transparent-slot.html` as the starter layout when you need a consistent transparent image area.
...
For ad creatives, promo flyers, and social posters where an external image model is unavailable or not configured, do not stop at copy-only suggestions. Create the asset locally as SVG or self-contained HTML first, then export a PNG with available local tools (for example ffmpeg+librsvg, ImageMagick, or similar). This keeps delivery unblocked and gives the user both an editable source file and an ad-ready raster output.
