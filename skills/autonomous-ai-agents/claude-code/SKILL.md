...
For Nusantara/ASPRI work, load the established Claude Code roster and runner notes in `references/nusantara-claude-multi-agent.md`. The user's preferred roster includes `agent-self-improving` as a durable process-learning layer in addition to frontend/backend/QA/infra agents.
...
13. **Production repo edits need re-read + verify discipline** — if a file was only sampled partially, re-read the full file before overwriting; after non-trivial patches, run the narrowest relevant test plus the project build. For feature removal, also remove route mounts, static routes, tests, vendor copies, and stale generated artifacts before declaring victory. See `references/repo-patch-and-verify.md` and `references/feature-removal-teardown.md`.
14. **File-based credentials need explicit path handling** — when a local key file exists, support mounted paths and multiple fallback locations; if env keys are stale or invalid, prioritize the file-backed source rather than assuming env wins.
15. **SDK abstractions can be the wrong first step** — if a provider library introduces typing/runtime friction, a direct HTTP call may be safer and easier to validate than forcing the SDK.
16. **Browser profile collisions are a real UI-test failure mode** — if Hermes browser automation fails with Chromium `ProcessSingleton` or socket-directory errors, switch to a standalone Playwright script with isolated temp `HOME`/`XDG_CONFIG_HOME`/`XDG_RUNTIME_DIR` and a known executablePath. See `references/browser-testing-workarounds.md`.

## UI verification after deploy
- For live dashboard checks, assert exact visible strings and save a screenshot from the same run.
- If the browser tool stalls or reuses a broken profile, prefer a deterministic Playwright script over repeated tool retries.
