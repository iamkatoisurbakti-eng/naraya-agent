...
7. **Local-first checkout recovery**: When a provider confirms payment but an upstream balance sync fails, prefer a safe local approval path over a hard rollback; log the provider failure, update the local transaction/saldo state, and let the user continue instead of blocking the payment.
...
Provider-backed payment bridges:
- When a user payment must also mutate a live provider wallet/ledger, resolve the provider identity first, auto-create the provider account if missing, then perform the balance mutation.
- Reconcile the local transaction only after the provider response is accepted; if the product rules allow it, log a warning and continue local approval for user-facing flows instead of hard-failing the entire checkout.
- Keep the local balance refresh path separate from the provider mutation path so you can retry one without duplicating the other.
- See `web-app-release-workflow/references/ksr888-gamexaglobal-transaction-sync.md` for the KSR888 implementation pattern.
